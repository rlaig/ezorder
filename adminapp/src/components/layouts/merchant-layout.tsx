import { Home, MenuIcon, ShoppingBag, QrCode, BarChart3, Settings, PanelLeft, User2 } from 'lucide-react';
...
const Logo = () => {
  return (
    <Link className="flex items-center text-white" to={paths.home.getHref()}>
      <img className="h-8 w-auto" src={logo} alt="EZOrder" />
...
export function MerchantLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation: SideNavigationItem[] = [
    { name: 'Dashboard', to: paths.merchant.dashboard.getHref(), icon: Home },
    { name: 'Menu', to: paths.merchant.menu.getHref(), icon: MenuIcon },
    { name: 'Orders', to: paths.merchant.orders.getHref(), icon: ShoppingBag },
    { name: 'QR Codes', to: paths.merchant.qrCodes.getHref(), icon: QrCode },
    { name: 'Analytics', to: paths.merchant.analytics.getHref(), icon: BarChart3 },
    { name: 'Settings', to: paths.merchant.settings.getHref(), icon: Settings },
  ];
